window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined" && !document.getElementById("dsh-auto-compact-style")) {
      ...
      style.textContent = [
        // 隐藏内置 ContextMeter 触发按钮（发送按钮旁的环形），由本插件唯一指示器替代
        'button[aria-label^="上下文已用"]{display:none!important}',
        'button[aria-label$=" of context used"]{display:none!important}',
        ...
        ".dac-compact-btn{width:100%;height:28px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-interactive-bg-hover-accent,#5a5f8a);color:var(--dsw-alias-label-primary);cursor:pointer;font-size:12px}",
